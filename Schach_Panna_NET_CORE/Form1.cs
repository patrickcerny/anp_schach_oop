...
namespace Schach_v1
{
    ...
    public partial class Form1 : Form
    {
        ...
        Color PossibleMoveColor = Color.ForestGreen;
        ...
        public void ResetGame()
        {
            ...
            foreach (Control item in Controls)
            {
                ...
                // Antwort: Dann würde unser INIT nich mehr gehen, da nich zuerst alle Felder und dann alle Figuren erzeugt werden,
                // sonder jedes Feld direkt mit seiner Figur erzeugt wird
        ...
        private void Tmr_Sekunde_Tick(object sender, EventArgs e)
        {
            ...
            // Antwort: KA ob i jetz lost bin aber Zeile _timeLeftWhte-- bzw _timeLeftBlack-- stoht doch oba oder?
        }

        void TimeRanOut(string color)
        {
            SpielPausieren();

            DialogResult answer = MessageBox.Show( color +  " hat verloren!", "Vorbei!", MessageBoxButtons.OK, MessageBoxIcon.Information);
            if (answer == DialogResult.Yes)
            {
                Close();
            }
...

        {
            ...
            if (_started)
            {
                ...
                else if (clickedFigure.FigureColor == CurrentPlayer)
                {
                    ...
                    {
                        tile.PaintSelfPossible(PossibleMoveColor);
                    }
                }
            }
        }
